<template>
    <div class="details_page">
    	<head-top goBack="true" :head-title="headTitle">
            ...
            <section slot="shocart" class="shocart_itemss" :class="{'move_in_cart':receiveInCart}" ref="cartContainer">
            	<router-link :to="{path: '/cart'}" id="doshaer"  class="icon-xiangqing_icon_gouwuche ciomsu" style="margin-right:1.5rem"></router-link>
            </section>
    	...
    	<div class="details_page_content">
    		<!--<div :class="{'move_in_cart':receiveInCart}" ref="cartContainer" class="llk">22</div>-->
    		<transition
	        appear
	        ...
	        >
	            ...
	            <span class="move_dot" v-if="item">
	            	<i style="display: inline-block; width:.8rem; height:.8rem; line-height:.8rem; color:#fff; background: #B4282D;border-radius: 50%; text-align: center; font-size: .46rem;">1</i>
	    ...
	    <div class="buycarts flex">
	    	...
	    	<div class="flex_v" :class="{gotopay_acitvity: minimumOrder <= 0}" @click="shoppingCart($event)">加入购物车</div>
...
export default {
    ...
    methods: {
        ...
        afterEnter(el){
        	let isu=document.getElementById('doshaer');
        	let sopwer=this.getOffsetRect(isu)
        	console.log(sopwer.left)
            el.style.transform = `translate3d(${sopwer.left/2}px,${'-'+(this.windowHeight+200)}px,0)`;
            el.children[0].style.transform = `translate3d(${sopwer.left/2}px,${'-'+(this.windowHeight+200)}px,0)`;
            el.style.transition = 'transform .55s cubic-bezier(1, -0.25, 0.4, -0.15)';
