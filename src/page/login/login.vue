<template>
    <div class="loginContainer" v-if="isShow">
...
<script>
    ...
    import {mobileCode, checkExsis, sendLogin,sendLogins, getcaptchas, accountLogin} from '../../service/getData'
    ...
    export default {
        ...
        //创建初始化ajax请求数据
        created(){
        	console.log('/wingoods/?id='+getStore('tabUrl'))
        	//this.$router.push('/wingoods/?id='+getStore('tabUrl'))
            //this.getCaptchaCode();
        ...
        methods: {
            ...
            async mobileLogin(){
                if (this.loginWay) {
                    ...
                    //手机号登录
                    this.userInfo = await sendLogin(this.phoneNumber,this.mobileCode);
                }
                //如果返回的值不正确，则弹出提示框，返回的值正确则返回上一页
                if(this.userInfo.success){
                	this.SER_USERINFOH(this.userInfo.object);
                	//alert(getStore('tabUrl'))
                    this.$router.push(getStore('tabUrl'))
                }else{
                	this.showAlert = true;
            ...
            async enshrine(){
                if(!this.$route.query.code&&!getStore('userId')){
                ...
                }else if(this.$route.query.code&&!getStore('userId')){
