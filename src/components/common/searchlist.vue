<template>
  <div class="shoplist_container">
    ...
    <ul v-load-more="loaderMore" v-if="shopListArr.length" type="1">
      <li class="searchlist" style="background-color:#fff" v-for="item in shopListArr">
        <router-link  :to="{path: 'details', query:{ goodsId: item.goodsId}}" tag='a' class="searchlist">
          <div class="user_info_com clearfix">
            <img :src="item.userInfo.wxHeadUrl" class="shop_imgs">
            <div class="info_box">
              <p class="user_info">{{item.userInfo.wxName + "(浏览" + item.userInfo.count + "次)"}}</p>
              <p class="detail_info">{{item.userInfo.userPhone}}</p>
            </div>
          </div>
        </router-link>
...
<script>
  ...
  import {searchUserBrowse} from 'src/service/getData'
  ...
  export default {
    ...
    methods: {
      async initData(){
        this.touchendtext= false;
        //获取数据
        let res = await searchUserBrowse(this.jskop,this.pagenum,this.querySize);
